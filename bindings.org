...
#+begin_src emacs-lisp
    ...
    (my/bind-leader "h k"   'my/which-key-debug "debug keybinding")
    (my/bind-leader "h r r" (lambda () (interactive) (my/reload-config)) "reload config")
  ...
  ;; Spell binding section removed — re-enable jinx first (see tools.org)
    ...
    ;; --- Jump (j) ---
    (my/bind-leader "i s" 'yas-insert-snippet "insert snippet")
    (my/bind-leader "i S" 'yas-insert-snippet "complete snippet")
    ...
    (my/bind-leader "j r" 'avy-resume "jump resume")
    (my/bind-leader "j i" 'evil-avy-goto-char-in-line "jump in line")
    ...
    ;; --- Open (o) ---
     (my/bind-leader "o t h" 'my/open-terminal-here "here (Alacritty)")
    (my/bind-leader "o p" 'my/popup-toggle "toggle popup")
     (my/bind-leader "o t p" 'ghostel-project "project (ghostel)")
     (my/bind-leader "o t a" 'eat "eat terminal")
     (my/bind-leader "o t t" 'ghostel-char-mode "ghostel char mode")
  ...
  ;; --- Run (r) ---
    ...
    ;; --- AI / LLM (a) ---
    (my/bind-leader "a a" 'gptel "gptel chat")
    (my/bind-leader "a s" 'gptel-send "gptel send")
    (my/bind-leader "a m" 'gptel-menu "gptel menu")
    (my/bind-leader "a r" 'gptel-rewrite "gptel rewrite")
    ;; --- ECA / Editor Code Assistant (A) ---
    ;; SPC A is ECA (coding agent); SPC a is gptel (general chat)
    (my/bind-leader "A A"   'eca "start ECA")
    (my/bind-leader "A c"   'eca-chat-toggle-window "toggle chat")
    (my/bind-leader "A s"   'eca-switch-to-chat "switch chat")
    (my/bind-leader "A n"   'eca-chat-new "new chat")
    (my/bind-leader "A k"   'eca-chat-clear "clear chat")
    (my/bind-leader "A r"   'eca-restart "restart server")
    (my/bind-leader "A m"   'eca-chat-select-model "select model")
    (my/bind-leader "A a"   'eca-chat-select-agent "select agent")
    (my/bind-leader "A x"   'eca-chat-stop-prompt "stop prompt")
    (my/bind-leader "A RET" 'eca-chat-send-prompt-at-chat "send prompt")
    (my/bind-leader "A SPC" 'eca-transient-menu "ECA menu")
    (my/bind-leader "A ,"   'eca-settings "settings")
    (my/bind-leader "A f"   'eca-chat-add-context-to-user-prompt "add context")
    (my/bind-leader "A d"   'eca-chat-drop-context-from-system-prompt "drop context")
    (my/bind-leader "A t a" 'eca-chat-tool-call-accept-all "accept all tools")
    (my/bind-leader "A t n" 'eca-chat-tool-call-accept-next "accept next tool")
    (my/bind-leader "A t r" 'eca-chat-tool-call-reject-next "reject next tool")
    ;; --- Other ---
