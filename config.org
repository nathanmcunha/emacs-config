...
#+begin_src emacs-lisp :tangle yes
    (use-package corfu
      ...
      (corfu-quit-no-match 'separator) ;; Keep popup open even when LSP filters everything
      :init
      (global-corfu-mode)
...
#+begin_src emacs-lisp :tangle yes
  (use-package cape
    :ensure t
    :init
    ;; Global fallbacks used in non-LSP buffers
    (add-hook 'completion-at-point-functions #'cape-dabbrev)
    (add-hook 'completion-at-point-functions #'cape-file)
    (add-hook 'completion-at-point-functions #'cape-keyword)
    :config
    ;; In Eglot buffers: mark the LSP capf as non-exclusive so tempel/file/dabbrev
    ;; can also contribute without interfering with Eglot's async completion.
    (defun my/eglot-capf-setup ()
      (setq-local completion-at-point-functions
                  (list (cape-wrap-nonexclusive #'eglot-completion-at-point)
                        #'tempel-expand
                        #'cape-file
                        #'cape-dabbrev)))
