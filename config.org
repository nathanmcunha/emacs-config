 ...
 #+begin_src emacs-lisp
   (use-package project
     ...
     (defun my/project-type (dir)
       "Return the project type for DIR (gradle, maven, npm, rust)."
       (cond
...
#+end_src
  ...
  ** Project Workspaces (Perspective Integration)
  ...
  #+begin_src emacs-lisp
    ...
    (advice-add 'project-switch-project :before #'my/project-workspace-switch)
  #+end_src
** Dirvish
#+begin_src emacs-lisp
  ...
  (use-package dirvish
    :ensure t
    :init
    ;; 1. Enable Dirvish globally (FIX: code was missing)
    (dirvish-override-dired-mode)
    
    :custom
    ;; Visual attributes (icons, git status, size, etc)
    ...
    ;; Default Dired adjustments
    (setq dired-listing-switches "-agho --group-directories-first"
          dired-recursive-deletes 'always
          dired-recursive-copies 'always
          delete-by-moving-to-trash t))

   ;; Function to open at project root
 ...
 ** Tree-Sitter Configuration
  Automatic tree-sitter grammar management using treesit-auto.
...
(use-package eglot-java
  ...
  ;; Define executable with Preview flags at process ROOT
  (setq eglot-java-eclipse-jdtls-executable 
        '("~/.local/share/mise/installs/java/temurin-25.0.2+10.0.LTS/bin/java" 
          "--enable-preview" 
          "-Xmx2G"))

  (defun my/eglot-java-init-opts (server jdt)
    (let ((debug-jar "/home/nathanmcunha/.config/emacs/java-debug/debug-plugin.jar")
          (java-home (expand-file-name "~/.local/share/mise/installs/java/temurin-25.0.2+10.0.LTS")))
      `(:bundles [,debug-jar]
        :extendedClientCapabilities (:classFileContentsSupport t
                                     :generateToStringPromptSupport t
                                     :supportsDebugger t)
        :settings (:java (:import (:gradle (:enabled t)) ;; Feche o :import aqui!
                          :jdt (:ls (:vmArgs ["--enable-preview" "-Xmx2G"]))
                          :configuration (:runtimes [(:name "JavaSE-25" 
                                                     :path ,java-home
                                                     :default t)]))))))
...
#+begin_src emacs-lisp 
  ...
  (defun my/java-debug-test-at-point ()
    ...
    (let* ((project-root (project-root (project-current)))
           (is-gradle (or (eq (my/project-type project-root) 'gradle)
...
#+begin_src emacs-lisp
   ...
   (defvar-local my/java-coverage-visible nil "Coverage state in buffer.")

  (defface my/jacoco-lens-face
    '((t :inherit shadow :slant italic :height 0.9 :foreground "green"))
     "Face for coverage percentage above methods.")
    ...
    (let ((gradle-path "build/reports/jacoco/test/jacocoTestReport.xml")
      ...
      (when project
        (let ((root (project-root project)))
          (cond
           ((file-exists-p (expand-file-name gradle-path root))
            (expand-file-name gradle-path root))
           ((file-exists-p (expand-file-name maven-path root))
   ...
   (defun my/java-coverage-browse-report ()
    ...
    (let ((gradle-html "build/reports/jacoco/test/html/index.html")
      ...
      (if project
          (let ((root (project-root project)))
            (cond
             ((file-exists-p (expand-file-name gradle-html root))
              (browse-url (expand-file-name gradle-html root)))
             ((file-exists-p (expand-file-name maven-html root))
  ...
  (defun my/java-apply-subtle-coverage ()
    ...
    (let* ((project (project-current))
           (report-path (my/java-get-jacoco-path))
           (relative-path (and project 
...
#+begin_src emacs-lisp
  (my-leader-def
    ...
    "SPC" '(execute-extended-command :which-key "M-x")
     ...
     "h b p" '(my/benchmark-current-project :which-key "benchmark project")
  ...
  ;; --- Explorer (E) ---
    ...
    "E e" '(my/dirvish-project :which-key "project root")
    "E f" '(dirvish-fd :which-key "find files (fd)")
    "E ." '(dirvish :which-key "current dir")
    
    ;; --- File (f) ---
    "f"   '(:ignore t :which-key "file")
    "f f" '(my/project-find-file :which-key "find file in project")
    ...
    "o t p" '(my/open-terminal-in-project :which-key "project")
     ...
     "p r" '(my/project-recent-files :which-key "recent files")
     "p c" '(my/project-compile :which-key "compile")
     "p t" '(my/project-test :which-key "run tests")
     "p x" '(my/project-run :which-key "run project")
     "p k" '(project-kill-buffers :which-key "kill buffers")
     "p d" '(project-dired :which-key "dired")
     "p v" '(project-vc-dir :which-key "vc dir")
     "p s" '(project-search :which-key "search (native)")
     "p /" '(consult-ripgrep :which-key "search (ripgrep)")
     "p !" '(project-shell-command :which-key "shell command")
     "p &" '(project-async-shell-command :which-key "async shell command")
     "p R" '(project-query-replace-regexp :which-key "query replace")
     "p w" '(my/project-workspace-switch :which-key "switch workspace")
     "p m" '(my/project-dispatch :which-key "project palette")
     "p B" '(my/benchmark-current-project :which-key "benchmark project")
