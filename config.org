...
Configuração leve usando Eglot, substituindo o lsp-java pesado.
...
(use-package eglot-java
  ...
  :config
  ;; Aumentar o timeout do eglot para dar tempo do JDTLS subir (comum em Java)
...
* Debugging (DAPE)
Configuração moderna de debug sem dap-mode.
...
#+begin_src emacs-lisp
  ...
  "Retorna o alvo do teste com o pacote completo (ex: com.foo.Bar.test)."
  (let* ((package-name (save-excursion
         ...
         ;; Monta o FQN (Full Qualified Name)
...
(is-gradle (format "*%s.%s" class-name clean-method))
  ...
  (defun my/java-debug-test-at-point ()
    "Inicializa a JVM em modo debug apenas para o método ou classe sob o cursor."
    (interactive)
    (unless (projectile-project-p)
      (user-error "Não estamos em um projeto Projectile"))
    
    (let* ((project-root (projectile-project-root))
           (is-gradle (or (string= (projectile-project-type) 'gradle)
                          (file-exists-p (expand-file-name "gradlew" project-root))))
           (test-target (my/get-java-full-test-target is-gradle))
           (buf-name " *java-test-debug-process*")
           ;; Constrói o comando específico
           (cmd (if is-gradle
      ...
      (message "Lançando JVM para: %s..." test-target)
        ...
        (set-process-filter
         ...
         (lambda (process output)
           ...
           (when (string-match-p "Listening for transport dt_socket" output)
             (message "JVM pronta para [%s]! Use o Dape para anexar." test-target)))))))
    
        (defun my/show-java-debug-log ()
              "Mostra o buffer de log do processo de debug atual."
              (interactive)
              (if (get-buffer " *java-test-debug-process*")
                  (display-buffer " *java-test-debug-process*")
                (message "Nenhum log de debug ativo.")))
          ...
          (add-to-list 'dape-configs
             `(java-attach
               ...
               ;; Aumentamos a resiliência da porta
               port (lambda ()
                      (let ((port (eglot-execute-command 
                                   (eglot-current-server) 
                                   "vscode.java.startDebugSession" nil)))
                        ;; Um breve respiro para o Sentinel do Eglot não se perder
                        (sit-for 0.2)
                        port))
               :type "java"
               :request "attach"
          ...
          :general
                  (my-leader-def
                    ...
                    "dt"  '(my/java-debug-test-at-point  :which-key "Debug Test")
                    ...
                    "dl"  '(dape-info-setup :which-key "Show Layout")
                    "dQ"  '(dape-kill :which-key "Quit Debugger")))
