...
#+begin_src emacs-lisp
    ...
    (tool-bar-mode -1)          ; Disable the toolbar (tooltip-mode -1)           ; Disable tooltips
...
#+end_src
* Flymake Configuration (Clean & IDE-Like)
Configuração para diagnósticos limpos e integração completa com Eglot/Consult.
...
#+begin_src emacs-lisp
  (use-package flymake
    ...
    ;; 2. O HOOK CORRIGIDO (A Mágica Visual)
    (add-hook 'flymake-project-diagnostics-mode-hook
              (lambda ()
                ;; Força BRUTALMENTE a remoção do cabeçalho
                (setq header-line-format nil) 
                (setq tabulated-list-use-header-line nil)
                
                ;; Remove setinhas de ordenação e números de linha
                (setq tabulated-list-gui-sort-indicator-mode nil)
                (display-line-numbers-mode 0)
                
                ;; Visual: Destaca a linha atual
                (hl-line-mode 1)))

    ;; 3. Função de Busca ROBUSTA (Corrigindo o erro 'listp')
    (defun my/consult-flymake-project ()
      "Busca erros do projeto via Consult, extraindo dados do Flymake."
      (interactive)
      ;; Garante que a lista existe (abre em background se precisar)
      ...
      (let ((diag-buffer (get-buffer "*Flymake diagnostics*")))
        (if (not diag-buffer)
          ...
          (with-current-buffer diag-buffer
            ;; --- CORREÇÃO DO ERRO 'listp' ---
            ;; No Emacs moderno, 'tabulated-list-entries' pode ser uma função.
            ;; Testamos com functionp e chamamos funcall se necessário.
            (let* ((entries-raw (if (functionp tabulated-list-entries)
              ...
              (dolist (entry entries-raw)
                ;; entry = (ID [VECTOR-STRINGS...])
                ;; O ID no Flymake é o próprio objeto 'flymake-diagnostic'
                (let* ((diag (car entry))
                       (buffer (flymake-diagnostic-buffer diag)))
                  
                  ;; Só mostramos erros de buffers válidos (para poder pular neles)
...
#+begin_src emacs-lisp
  (use-package cape
    ...
    ;; Função mágica para fazer o TAB funcionar com Yasnippet E Corfu
    (defun my/yas-try-expand-first ()
      "Tenta expandir o snippet primeiro. Se não conseguir, roda o comando original (ex: corfu-next ou indent)."
      (interactive)
      (unless (yas-expand)
        (if (and (boundp 'corfu-mode) corfu-mode (frame-live-p corfu--frame))
            (corfu-next)
          (indent-for-tab-command))))

    ;; Rebind do TAB no mapa global ou do modo de prog
...
** Eletric
#+begin_src emacs-lisp
;; Electric Pair: O jeito nativo e leve de fechar parênteses
(use-package elec-pair
  :ensure nil
  :hook (prog-mode . electric-pair-mode)
  :config
  ;; Desabilita o fechamento de '<' para não atrapalhar comparações em Java (i < 10)
  ;; Mas mantém funcional se você digitar um par completo (dependendo do modo)
...
+begin_src emacs-lisp
  ;; 1. Desativa o Flymake (para não ter dois sublinhados na tela)
  (use-package flymake
    :config
    ;; Garante que o Flymake não ligue sozinho
    (setq flymake-start-on-save-buffer nil)
    (setq flymake-no-changes-timeout nil))

  ;; 2. Configura o Flycheck (A engine visual)
  (use-package flycheck
    ...
    ;; Aumenta um pouco o delay para não pesar
    (setq flycheck-display-errors-delay 0.25)
    (setq flycheck-idle-change-delay 0.5)

    ;; Define a lista de erros para aparecer embaixo (20% da altura)
    (add-to-list 'display-buffer-alist
                 `(,(rx bos "*Flycheck errors*" eos)
                   ...
                   (window-height   . 0.20))))
  ;; 3. Consult Flycheck (Pacote Separado!)
  (use-package consult-flycheck
    :ensure t  
    :after (consult flycheck))

  ;; 3. A Ponte (Eglot -> Flycheck)
  (use-package flycheck-eglot
    ...
    ;; (Opcional) Limpeza do texto "Java [123]"
    ;; O Flycheck permite alterar a mensagem antes de exibir
    (defun my/flycheck-java-cleaner (errors)
      (dolist (err errors)
        (let ((msg (flycheck-error-message err)))
          (when (string-match "^Java \\[[0-9]+\\]: \\(.*\\)" msg)
            (setf (flycheck-error-message err) (match-string 1 msg)))))
      errors)
    
    (add-hook 'flycheck-after-syntax-check-hook
              (lambda ()
                (my/flycheck-java-cleaner flycheck-current-errors))))

  ;; 4. Atalhos (General)
  (my-leader-def
    "e"   '(:ignore t :which-key "errors")
    ;; A lista nativa do Flycheck (Bonita, colunas alinhadas)
    "e p" '(flycheck-list-errors :which-key "List Errors (Panel)")
    
    ;; Busca com Consult (Agora usando suporte nativo do Flycheck)
    "e l" '(consult-flycheck :which-key "Search Errors (Consult)")
    
    ;; Navegação
    "e n" '(flycheck-next-error :which-key "Next")
    "e N" '(flycheck-previous-error :which-key "Prev"))
#+end_src
...
(use-package yasnippet
  ...
  :config
  ;; Define seus snippets pessoais
  (setq yas-snippet-dirs (list (expand-file-name "snippets" user-emacs-directory)))
  
  ;; Comportamento do TAB (para coexistir com Corfu/Company)
...
(use-package yasnippet-snippets
  ...
  :after yasnippet
  :config
  ;; Garante que o diretório do pacote seja adicionado
  (require 'yasnippet-snippets) 
  (add-to-list 'yas-snippet-dirs yasnippet-snippets-dir t)
  
  ;; O PULO DO GATO: Recarrega tudo AGORA, depois que ambos os diretórios estão configurados
  (yas-reload-all))
