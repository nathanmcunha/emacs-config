...
#+begin_src emacs-lisp
  (use-package dirvish :ensure t :init
    ;; Habilita o Dirvish globalmente (substituindo o Dired padrão)
    (dirvish-override-dired-mode)
    :custom ;; Atributos visuais para buffers normais (Dired Full)
    (dirvish-attributes '(vc-state nerd-icons file-size collapse subtree-state git-msg))
  ;; Configuração da Barra Lateral (Side)
  (dirvish-side-width 30)
  (dirvish-side-display-alist `((side . left) (slot . -1))) ;; Força sempre à esquerda extrema

  :config
  ;; Habilita preview de arquivos e "follow mode" (sincronizar com buffer aberto)
  (dirvish-peek-mode)
  (dirvish-side-follow-mode)

  ;; --- VISUAL ESTILO TREEMACS ---
  ;; Na barra lateral, mostramos APENAS ícones e nome (sem tamanho, data, etc)
  (setq dirvish-side-attributes '(nerd-icons collapse subtree-state))

  ;; Ajustes finos do Dired base
