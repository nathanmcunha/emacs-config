...
;; Terminal Launchers
(defun my/launch-alacritty (dir) (call-process-shell-command (format " /home/nathanmcunha/.nix-profile/bin/alacritty --working-directory %s" (shell-quote-argument dir)) nil 0))
...
#+begin_src emacs-lisp
  ...
  (defun my/run--collect-candidates ()
    ...
    (let ((all (append (my/run--mise-tasks)
                       ...
                       my/run-favorites
                       my/run-history)))
      (delete-dups (seq-filter #'stringp all))))
...
#+begin_src emacs-lisp
  ...
  (add-hook 'project-switch-hook #'my/cape-run--invalidate-cache)
    ...
    (use-package embark
      ...
      (defvar my/embark-identifier-map
        (let ((map (copy-keymap embark-identifier-map)))
          ...
          (define-key map (kbd "O") #'eglot-organize-imports)
...
#+begin_src emacs-lisp
  ...
  ;; === Java Class Creation (IntelliJ-style) ===
  ...
  (defun my/java-create-class (fqcn kind)
    "Create a Java file for FQCN (e.g. com.foo.MyClass) of KIND (class/interface/enum/record/annotation)."
  ...
  (defun my/java-new-class ()
    "Create a new Java class, prompting for fully qualified name."
    (interactive)
  ...
  ;; === IntelliJ-like Utilities via JDTLS ===

  (defun my/java-generate-toString ()
    "Generate toString() via JDTLS code action."
    (interactive)
    (eglot-code-actions nil nil "source.generate.toString" t))

  (defun my/java-generate-hashCodeEquals ()
    "Generate hashCode/equals via JDTLS."
    (interactive)
    (eglot-code-actions nil nil "source.generate.hashCodeEquals" t))

  (defun my/java-generate-constructors ()
    "Generate constructors via JDTLS."
    (interactive)
    (eglot-code-actions nil nil "source.generate.constructors" t))

  (defun my/java-generate-getters-setters ()
    "Generate accessors via JDTLS."
    (interactive)
    (eglot-code-actions nil nil "source.generate.accessors" t))

  (defun my/java-organize-imports ()
    "Organize imports via JDTLS."
    (interactive)
    (eglot-code-actions nil nil "source.organizeImports" t))

  (defun my/java-extract-variable ()
    "Extract selection to a local variable."
    (interactive)
    (eglot-code-actions (region-beginning) (region-end) "refactor.extract.variable" t))

  (defun my/java-extract-method ()
    "Extract selection to a method."
    (interactive)
    (eglot-code-actions (region-beginning) (region-end) "refactor.extract.function" t))

  (defun my/java-extract-constant ()
    "Extract selection to a constant."
    (interactive)
    (eglot-code-actions (region-beginning) (region-end) "refactor.extract.constant" t))

  (defun my/java-find-implementations ()
    "Find implementations of interface/abstract method at point."
    (interactive)
  ...
  (defun my/java-go-to-test ()
    ...
    (let* ((file (buffer-file-name))
      ...
      (let ((target (if is-test
                        (replace-regexp-in-string "/src/test/" "/src/main/" (replace-regexp-in-string "Test\\.java$" ".java" file))
                      (replace-regexp-in-string "/src/main/" "/src/test/" (replace-regexp-in-string "\\.java$" "Test.java" file)))))
...
#+begin_src emacs-lisp
    ;; Nix mode (for nixd LSP)
    (use-package nix-mode
      :ensure t)

    ;; QML mode (for qmlls LSP)
    ...
    (use-package eglot
      ...
      ;; 7. Nix: nixd (Nix Language Daemon)
      ;; Install: nixd via home.packages (already in emacs.nix)
      (add-to-list 'eglot-server-programs
                   '((nix-mode) . ("nixd")))

      ;; 8. QML: qmlls (Qt QML Language Server)
      ;; Install: qtbase.dev via home.packages
      (add-to-list 'eglot-server-programs
                   '((qml-mode) . ("qmlls")))
      ...
      ;; Reconnect eglot when envrc activates a new environment (picks up mise venv/PATH)
...
#+begin_src emacs-lisp
  ...
  (my/bind-leader "c j g t" 'my/java-generate-toString "generate toString")
  (my/bind-leader "c j g h" 'my/java-generate-hashCodeEquals "generate hashCode/equals")
  (my/bind-leader "c j g c" 'my/java-generate-constructors "generate constructor")
  (my/bind-leader "c j g a" 'my/java-generate-getters-setters "generate accessors")

  ;; Refactor (IntelliJ Ctrl+Alt+V/M style)
  (my/bind-leader "c j x v" 'my/java-extract-variable "extract variable")
  (my/bind-leader "c j x m" 'my/java-extract-method "extract method")
  (my/bind-leader "c j x c" 'my/java-extract-constant "extract constant")
